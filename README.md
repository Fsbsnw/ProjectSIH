...
- `GameInstanceSubsystem` 기반의 사건 진행 상태 관리
- 조사 → 회의 → 파티 편성 → 전투 → 결과 페이즈 전환
- 맵 이동 후 Ready 메시지를 기다렸다가 각 페이즈를 안전하게 시작
- 완료, 스킵, 전투 결과 메시지를 통한 시스템 간 결합도 완화
- 사건 ID, 현재 페이즈, 파티 구성 등 컨텍스트 유효성 검증
